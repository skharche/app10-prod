	...
	class suite
	{
		...
		*			<ul>
		...
		function getAvailableOfficeSpaceDetails()
		{
			...
			$q = " SELECT tsuite.*, DATE_FORMAT(tsuite.date_available, '%M %e, %Y') as date_available2, tsuiteimages.image_name, tsuiteimages.image_path, tfloors.number, tfloors.floor_height, tf2.floor_height as floor_height2, tf2.extruded_height, (tbuilding.altitude / tbuilding.floors) as building_floor_height, tbuilding.floors, tbuilding.yearbuilt, tbuilding.lastreno, YEAR(CURDATE()) - yearbuilt AS year_difference, tbuilding.basefloorheight, tbuilding.address, tbuilding.total_additional_rent, tbuilding.idtsubmarket, tsubmarket.ssubname, tbuilding.sbuildingname, tbuilding.class, tbuilding.latitude, tbuilding.longitude, CAST(REPLACE(tbuilding.grossofficearea, ',', '') AS UNSIGNED) as grossofficearea, tcompany.companyname AS companyname_long, tcompany.shortcompanyname AS companyname, tcompany.companyimage, tcoords.coords
