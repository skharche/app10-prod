...
function prepareCityGridStructure(citiesGrid)
{
	...
	*/
	...
	headerContent += '<hr />';
	headerContent += '<span class="badge" style="background-color: '+classColor['Office']+'">Office</span><span class="badge" style="background-color: '+classColor['Apartments']+'">Residential</span><span class="badge" style="background-color: '+classColor['Retail']+'">Retail</span><span class="badge" style="background-color: '+classColor['Hotel']+'">Hotels</span><span class="badge" style="background-color: white;">Other</span>';
	headerContent += '</div>';
	...
	*/
	...
	$(".container").append("<br /><hr /><a href='http://www.floorplan.city' style='float:left;'>http://www.floorplan.city</a><br /><br />");
}
...
function getVisualizationDropdown(idtcity)
{
	...
	str = "<select style='width:100%;' onChange='setCookie(this.id, this.value);' class='' id='cityVis"+idtcity+"'>";
		...
		$.each(buildingTypeDropdown, function (index, eachType){
			...
			if(idtcity == 23 && eachType == "Floorplan")
			{
				str += "<option value='AvailableOfficeSpace'>Example Market Vacancy</option>";
				str += "<option value='OfficeRentalRates'>Example Market Rates</option>";
			}
			
			if(idtcity == 2 && eachType == "Floorplan")
			{
...
function openVisualizationForCity(cityDetails)
{
	...
	setLoadingMessage(cityDetails.cnt, marketDetailsV2[lastMarketLoaded].smarketname);
...
function setLoadingMessage(cnt, cityName)
{
	console.log("setLoadingMessage("+cnt+", "+cityName);
	if(typeof cityName != "undefined" && cityName != "" && cnt > 0 && cnt != "")
		$(".loading-message").html("Loading "+numberWithCommaWithoutDecimal2(cnt)+" Properties in "+cityName+"...");
	else
...
function loadMarket(val)
{
	...
	setTimeout(function (){
		...
		if(lastSelectedBuildingType == "AvailableOfficeSpace" || lastSelectedBuildingType == "OfficeRentalRates" || lastSelectedBuildingType == "InvestmentSalesMarket")
		{
			lastSelectedBuildingType = "Office";
		}
...
function getBuildingData(marketDetails, cameraChange = true)
{
	...
	setTimeout(function (){ $(".loading-overlay").fadeOut(); }, 3000);
	...
	setTimeout(function () {
		...
		if(true)// Add condition here for any one effect active
		{
			...
			if(typeof defaultCamera != "undefined" && typeof defaultCamera[0] != "undefined")
			{
				...
				flyToCameraView(defaultCamera[0], defaultCamera[1], defaultCamera[2], defaultCamera[3], defaultCamera[4], defaultCamera[5], 4);
				defaultCamera = [];
			}
		}
	}, 1000);
...
function clearPrimitives( clearFog = true )
{
	if(typeof RemoveEntitiesByType != "undefined")
...
function ShowInfobox(idtbl, id)
{
	...
	*/
	...
	else if(lastSelectedBuildingType == "Office" || (lastSelectedBuildingType == "All" && hotelClasses.includes(TempBldgData[idtbl].buildingclass)))
	{
		var str = "<table class='table table-striped minPaddingtable' cellpadding=2 cellspacing=0 border=0 witdh='90%'>";
		//str += "<tr><td>Name</td><td>"+TempBldgData[idtbl].sbuildingname+"</td></tr>";
		str += "<tr><td colspan=2 class='infoboxBuildingAddress'>"+TempBldgData[idtbl].address+"</td><td colspan='2' class='alignRight' class='infoboxSubmarketName'><a href=\"javascript:flyToSubmarketCamera("+TempBldgData[idtbl].idtsubmarket+");\">"+TempBldgData[idtbl].ssubname+"</a></td></tr>";
		...
		str += "<tr><td>Total&nbsp;Parking&nbsp;</td><td>"+numberWithCommaWithoutDecimal(TempBldgData[idtbl].parkingstalls, "")+"</td><td>Additional&nbsp;Rent&nbsp;</td><td>"+numberWithCommaWithTwoDecimal(TempBldgData[idtbl].total_additional_rent, "$", "/psf")+"</td></tr>";
		...
		if(parseInt(TempBldgData[idtbl].total_available_office_area) > 0)
		{
			...
			str += "<tr><td>Vacancy</td><td>"+calcPercentage+"%</td><td>Available</td><td>"+numberWithCommaWithoutDecimal(parseInt(TempBldgData[idtbl].total_available_office_area), " ", " "+cityAreaMeasurementUnit)+"</td></tr>";
		}
		else
		{
			str += "<tr><td>Vacancy</td><td>0%</td><td>Available</td><td></td></tr>";
		}
	...
	*/
	...
	str += "<span class='pull-left'><button class='btn btn-sm btn-secondary actionButtons' id='highlightButton' onClick=\"createFloorsEffect("+idtbl+");\">Floors</button><button class='btn btn-sm btn-secondary actionButtons ' id='assetButton' onClick=\"createBuildingAssets("+idtbl+");\">Assets</button><button class='btn btn-sm btn-secondary actionButtons' id='floorplanButton' onClick=\"getDataForFloorPlan("+idtbl+");\">Floorplans</button><button class='btn btn-sm btn-secondary actionButtons ' style='display:none;' id='cameraRotation2' onClick=\"ToggleCameraRotationForPoint2();\">Cam2</button></span>";
	...
	str += "<span class='pull-right' style='cursor:pointer; float: right; margin-top: 10px; '><span  id='copyURLButton' ><img src='images/link_24.png' height='32px;' width='32px;' /></span></span>";
...
function ShowInfoboxOfficeMarketSales(idtbl)
{
	...
	st += "<span style='position: absolute; right: 2%; font-weight: none !important;'>"+TempBldgData[idtbl].idtbuilding+"</span>";
	st += "";
	$(".infoboxHeaderData").html(st);
	...
	*/
	str += "<tr>";
		str += "<td>Sale Date</td><td>"+printIfNotNull(TempBldgData[idtbl].sale_date)+"</td>";
		str += "<td>Sale Price</td><td>"+numberWithCommaWithoutDecimal(TempBldgData[idtbl].sold_price, "$")+"</td>";
	str += "</tr>";
	
	str += "<tr>";
		str += "<td>Type</td><td style='background-color:"+classColor[TempBldgData[idtbl].office_conversion.replace(' ', '')]+"'>"+printIfNotNull(TempBldgData[idtbl].office_conversion)+"</td>";
		var psf = TempBldgData[idtbl].sold_price / parseInt(TempBldgData[idtbl].grossofficearea);
		if(psf > 0)
			str += "<td>Price PSF</td><td>$"+psf.toFixed(2)+"</td>";
		else
...
function ShowLegend()
{
	if(isMobile.any() == null)//Desktop
	{
		...
		{
			str += '<div class="colorLegend2 " style="width: 75px !important; background-color: '+classColor["AA"]+';">Office AA</div>';
			str += '<div class="colorLegend2 " style="width: 75px !important; background-color: '+classColor["A"]+';">Office A</div>';
			str += '<div class="colorLegend2 " style="width: 75px !important; background-color: '+classColor["B"]+';">Office B</div>';
			str += '<div class="colorLegend2 " style="width: 75px !important; background-color: '+classColor["C"]+';">Office C</div>';
		}
		...
		{
			str += '<div class="colorLegend2 " style="width: 115px; background-color: '+classColor["AA"]+';">Office AA</div>';
			str += '<div class="colorLegend2 " style="width: 115px; background-color: '+classColor["A"]+';">Office A</div>';
			str += '<div class="colorLegend2 " style="width: 115px; background-color: '+classColor["B"]+';">Office B</div>';
			str += '<div class="colorLegend2 " style="width: 115px; background-color: '+classColor["C"]+';">Office C</div>';
		...
		else if(lastSelectedBuildingType == "OfficeRentalRates")
		{
			$.each(window.priceSQMRange, function (index, eachColor){
				...
				str += '<div class="colorLegend3 " style=" background-color: '+eachColor.color+';">'+displayText+'</div>';
			});
		}
...
function ShowSummaryInfobox()
{
	...
	$.each(buildingTypeDropdown, function (index, eachType){
		...
		if(lastMarketLoaded == 36 && eachType == "Floorplan")
		{
			dropdownSelected = " ";
			if(lastSelectedBuildingType == "AvailableOfficeSpace")
				dropdownSelected = " selected ";
			buildingTypeOption += "<option "+dropdownSelected+" value='AvailableOfficeSpace'>Example Market Vacancy</option>";
		...
		if(lastMarketLoaded == 1 && eachType == "Floorplan")
		{
			dropdownSelected = " ";
	...
	else if(lastSelectedBuildingType == "Hotel")
	{
		if(hotelSummaryDetails.length > 0 && typeof hotelSummaryDetails[lastMarketLoaded] != "undefined")
		{
			...
			str += "<td class='alignCenter'>Occupancy</td><td class='alignCenter'>Avg Nightly Rate</td></tr>";
			var totalBlgs = 0;
			var totalUnits = 0;
			...
			str += "<tr><td>Total</td><td  class='alignCenter'>"+printNumberFormat(totalBlgs)+"</td><td class='alignRight'>"+printNumberFormat(totalUnits)+"</td><td></td><td></td></tr>";
		}
	}
	else if(lastSelectedBuildingType == "Development")
	{
		if( developmentBuildingSummary.length > 0 && typeof developmentBuildingSummary[lastMarketLoaded] != "undefined" && typeof developmentBuildingSummary[lastMarketLoaded]["PROPOSED"] != "undefined" )
		{
			...
			str += "<tr><td>Total</td><td class='alignCenter'>"+printNumberFormat(totalBlgs)+"</td><td class='alignCenter'>"+printNumberFormat(totalUnits)+"</td><td class='alignCenter'>"+printNumberFormat(totalHotelDoors)+"</td><td class='alignCenter'>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(totalOfficeArea), "", "")+"</td></tr>";
		}
		else
		{
			str = "";
	...
	else if(lastSelectedBuildingType == "All")
	{
		...
		{
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['AA']+";"+cellWidth+"'>Office AA</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["AA"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["AA"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["AA"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['A']+";"+cellWidth+"'>Office A</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["A"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["A"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["A"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['B']+";"+cellWidth+"'>Office B</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["B"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["B"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["B"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['C']+";"+cellWidth+"'>Office C</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["C"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["C"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["C"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['Retail']+";"+cellWidth+"'>Retail</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["Retail"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["Retail"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["Retail"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['MDU']+";"+cellWidth+"'>Condominiums</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["MDU"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["MDU"][1]+"</td><td>"+numberWithCommaWithoutDecimal(summaryDetails[lastMarketLoaded]["Condominiums"].officeArea, "", " units")+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['Apartments']+";"+cellWidth+"'>Apartments</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["APT"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["APT"][1]+"</td><td>"+numberWithCommaWithoutDecimal(summaryDetails[lastMarketLoaded]["Apartments"].officeArea, "", " units")+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['HOTEL']+";"+cellWidth+"'>Hotels</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["HOTEL"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["HOTEL"][1]+"</td><td>"+numberWithCommaWithoutDecimal(summaryDetails[lastMarketLoaded]["Hotel"].officeArea, "", " doors")+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['SENIOR']+";"+cellWidth+"'>Retirement</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["SENIOR"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["SENIOR"][1]+"</td><td>"+numberWithCommaWithoutDecimal(summaryDetails[lastMarketLoaded]["SENIOR"].officeArea, "", " units")+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['EDU']+";"+cellWidth+"'>Education</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["EDU"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["EDU"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["EDU"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['MED']+";"+cellWidth+"'>Healthcare</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["MED"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["MED"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["MED"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['GOV']+";"+cellWidth+"'>Government</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["GOV"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["GOV"][1]+"</td><td>"+numberWithCommaWithoutDecimal(getAreaInCityUnits(summaryDetails[lastMarketLoaded]["GOV"].officeArea), "", " "+cityAreaMeasurementUnit)+"</td></tr>"; 
			str += "<tr><td class='infoboxLegendTD' style='background-color:"+classColor['PRKS']+";"+cellWidth+"'>Parkades</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["PRKS"][0]+"</td><td  class='alignCenter'>"+allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["PRKS"][1]+"</td><td>"+numberWithCommaWithoutDecimal(summaryDetails[lastMarketLoaded]["PRKS"].stalls, "", " stalls")+"</td></tr>"; 
			str += "<tr><td>Total</td><td class='alignCenter'>"+numberWithCommaWithoutDecimal(allBuildingVisualizationSummary[parseInt(lastMarketLoaded)]["TOTAL"][0])+"</td><td></td></tr>"; 
		}
		str += "</table>";
	...
	else if(lastSelectedBuildingType == "Office")
	{
		...
		if(typeof summaryDetails[lastMarketLoaded] != "undefined")
		{
			...
			*/
				...
				str += "<tr><td>Total</td><td class='alignCenter'>"+printNumberFormat(totalBuildings)+"</td><td  class='alignCenter'>"+printNumberFormat(totalArea)+"</td>";
				...
				str += "<td class='alignCenter'>"+Math.round(totalYearDifference/4)+"</td>";
			...
			if(lastSelectedBuildingType == "Office" || lastSelectedBuildingType == "Residential")
				str += "<tr><td>Total</td><td class='alignCenter'>"+printNumberFormat(totalBuildings)+"</td><td style='float:right;'>"+printNumberFormat(totalArea.toFixed(2))+"</td><td></td><td></td></tr>";
			else
				str += "<tr><td>Total</td><td class='alignCenter'>"+printNumberFormat(totalBuildings)+"</td><td style='float:right;'>"+printNumberFormat(totalUnits)+"</td><td></td><td></td></tr>";
			*/
		}
	}
	else if(lastSelectedBuildingType != "Floorplan" )
		...
		if(true)
		{
			...
			else
			{
				if(cityAreaMeasurementUnit.toLowerCase() == "sqm" && lastSelectedBuildingType == "Office")
					totalArea = parseFloat(totalArea) * parseFloat(cityAreaMeasurementMultiplier);
				totalArea = Math.round(totalArea);
				str += "<tr><td>Total</td><td class='alignCenter'>"+printNumberFormat(totalBuildings)+"</td><td style='float:right;'>"+printNumberFormat(totalArea)+"</td><td class='hiddenGrayField' style='padding-left: 20px !important;'>[Hidden]</td><td class='hiddenGrayField' style='padding-left: 20px !important;'>[Hidden]</td></tr>";
				str += "</table>";
			}
			/*
			if(lastSelectedBuildingType == "Office" || lastSelectedBuildingType == "Residential")
...
function createSummaryInfoboxForAvailableOfficeSpace()
{
	...
	str += "<tr>";
		...
		str += "<td class='alignCenter' style='padding-right:20px !important;'>"+printNumberFormat(Math.round(parseFloat(window.availableOfficeSpaceSummary["Direct"].availableOfficeArea)))+"</td>";
		var directVacancy = ((parseFloat(window.availableOfficeSpaceSummary["Direct"].availableOfficeArea) / parseInt(window.totalOfficeAreaForVacancy)) * 100).toFixed(2);
	...
	str += "<tr>";
		...
		str += "<td class='alignCenter' style='padding-right:20px !important;'>"+printNumberFormat(Math.round(parseFloat(window.availableOfficeSpaceSummary["Sublease"].availableOfficeArea)))+"</td>";
		var subLeaseVacancy = ((parseFloat(window.availableOfficeSpaceSummary["Sublease"].availableOfficeArea) / parseInt(window.totalOfficeAreaForVacancy)) * 100).toFixed(2);
	...
	str += "<tr>";
		...
		str += "<td class='alignCenter' style='padding-right:20px !important;'>"+printNumberFormat(Math.round(parseFloat(window.availableOfficeSpaceSummary["Co-Working"].availableOfficeArea)))+"</td>";
		var coWorkingVacancy = ((parseFloat(window.availableOfficeSpaceSummary["Co-Working"].availableOfficeArea) / parseInt(window.totalOfficeAreaForVacancy)) * 100).toFixed(2);
	...
	str += "<tr>";
		...
		var totalAvailableSQM = parseFloat(window.availableOfficeSpaceSummary["Direct"].availableOfficeArea) + parseFloat(window.availableOfficeSpaceSummary["Sublease"].availableOfficeArea) + parseFloat(window.availableOfficeSpaceSummary["Co-Working"].availableOfficeArea);
		...
		if(!isNaN(totalAvailableSQM))
			str += ( (parseInt(totalAvailableSQM) / parseInt(window.totalOfficeAreaForVacancy)) * 100 ).toFixed(2)+"%";
		else
			str += "0%";
		str += "</td>";
	str += "</tr>";
...
function createSummaryInfoboxForCalgaryOfficeMarketSales()
{
	var str = "<table class='table table-striped minPaddingtable' cellpadding=2 cellspacing=0 border=0 witdh='90%'>";
	str += "<tr><td colspan='5'></td></tr>";
	str += "<tr><td>Asset Type</td><td class='alignCenter'>Properties</td>";
	str += "<td class='alignCenter'>Sq Ft</td>";
	str += "<td class='alignCenter'>Total Value</td>";
	str += "</tr>";
	...
	$.each(window.calgaryOfficeSaleSummary, function (index, row){
		...
		str += "<tr>";
			str += "<td style='width: 115px; background-color:"+classColor[row.office_conversion.replace(' ', '')]+"'>"+row.office_conversion+"</td>";
...
function loadSydneyMarketDropdown(cityId, marketId, extraField = false)
{
	...
	$.each(buildingTypeDropdown, function (index, eachType){
		...
		if((marketId == 36 || cityId == 23) && eachType == "Floorplan")
		{
			$('#marketDropdown')
			 .append($("<option></option>")
			 .attr("value","AvailableOfficeSpace")
			 .text("Example Market Vacancy"));
		...
		else if((marketId == 1 || cityId == 2) && eachType == "Floorplan")
		{
			$('#marketDropdown')
			 .append($("<option></option>")
		...
		//var latLonDetails = getMapCenterV2();
		  ...
		  */
		  ...
		  unsubscribeSlowZoom = viewer.clock.onTick.addEventListener(() => {
			...
			//debugger;
			const SMOOTHNESS = 3500; //it would make one full circle in roughly 800 frames
...
function createBuildingAssets(idtbldg)
{
	...
	if(!buildingAssetEffectActive)
	{
		...
		$.each(TempBldgData[idtbldg].floorDetails, function(i, eachFloor){
			if (clr == "" || clr == "Cesium.Color.YELLOW.withAlpha(0.07)")
...
function clearBuildingAssets()
{
	...
	$.each(TempBldgData[lastBuildingSolidFloorHighlighted].floorDetails, function(i, eachFloor){
		viewer.entities.removeById("floor-"+lastBuildingSolidFloorHighlighted+"-"+(i+1));
	});
	if(effectsArray[4] == 0)
	{
...
function clearIsolateSatelliteEffect()
{
...
function createDarkOverlayEffect(foceClear = false)
{
	...
	else
	{
		...
		if(!darkOverlayEffectActive)
		{
			...
			eval("viewer.entities.add({ id: 'FogEffectEntity', polygon: { hierarchy: { positions: Cesium.Cartesian3.fromDegreesArray("+cityBoundaries[lastCityLoaded]+"), holes: eval(["+window.lastHolesString+"]) }, material: Cesium.Color.BLACK.withAlpha(0.5), classificationType: Cesium.ClassificationType.CESIUM_3D_TILE, }, }) ");
...
function createWhiteOverlayEffect(foceClear = false)
{
	...
	else
	{
		...
		if(!whiteOverlayEffectActive)
		{
			...
			eval("viewer.entities.add({ id: 'FogEffectEntity', polygon: { hierarchy: { positions: Cesium.Cartesian3.fromDegreesArray("+cityBoundaries[lastCityLoaded]+"), holes: eval(["+window.lastHolesString+"]) }, material: Cesium.Color.WHITE.withAlpha(0.5), classificationType: Cesium.ClassificationType.CESIUM_3D_TILE, }, }) ");
			window.darkOverlayEffectColor = 'WHITE';
...
function prepareFloorPlanInInfobox(idtbldg, floorNumber, details)
{
	...
	if(lastSelectedBuildingType == "Floorplan")
	{
		...
		st += "<span class='pull-right' style='cursor:pointer; position: absolute; right: 2% !important; margin-top: -5px !important;'><span id='copyURLButton' ><img src='images/link_24.png' width='32px;' height='32px;' /></span></span>";
		st += '<br />';
		st += '</div>';
	}
	...
	if(details.length == 1)
	{
		...
		{
			st += "&nbsp; Area: <span class='buildingNameOnInfobox'>"+numberWithCommaWithoutDecimal(details[0]["suite_area"], "", " "+cityAreaMeasurementUnit)+"</span>";
		}
	...
	else
	{
		...
		$.each(details, function (index, eachSuite){
			...
			tabContent += '<div class="tab-content '+isActive+'" id="tab-'+index+'">';
				tabContent += "Area: <span class='buildingNameOnInfobox'>"+numberWithCommaWithoutDecimal(eachSuite.suite_area, "", " "+cityAreaMeasurementUnit)+"</span>";
...
function prepareAvailableOfficeSpaceInfobox(idtbldg, index, details2, allSuitesOnFloor)
{
	...
	if(allSuitesOnFloor.length == 1)
	{
		...
		if(details[0]["suite_area"] != "" && parseInt(details[0]["suite_area"]) > 0)
		{
			st += "<td>Available Area</td><td><b>"+numberWithCommaWithoutDecimal(details[0]["suite_area"], "", " "+cityAreaMeasurementUnit)+"</b></td>";
		}
		else
			st += "<td></td><td></td>";
		st += "</tr>";
		...
		st += "<tr><td>Suite</td><td>"+details[0]["suite_name"]+"</td><td>Currently Vacant</td><td></td></tr>";
		
		st += "<tr><td>Class</td><td>"+details[0]["class"]+"</td><td>Price per Sq Ft</td><td></td></tr>";
		
		//Created  // "+PrintOnlyDate(PrintIfNotNull(details[0].date_created))+"
		st += "<tr><td onClick=\"openFullScreenImage('70', '24', '1103', '', '', '100', 'Yes');\">Age</td><td>"+details[0]["year_difference"]+"</td><td>Annual Rent</td><td></td></tr>";
		
		//Status //Active
		st += "<tr><td></td><td></td><td>Lease Type</td><td style='background-color:"+classColor[details[0]["lease_type"]]+"'>"+details[0]["lease_type"]+"</td></tr>";
		...
		st += "<tr><td colspan=2>Listing Broker</td><td></td><td aligh='right' style='float:right'>&nbsp;<span style='cursor:pointer;margin: 0px !important;padding-right:0px !important;' id='copyURLButton' ><img width='32px;' height='32px;' src='images/link_24.png' /></span></td></tr>";
		st += "</table>";
		$(".infoboxContainerData").html(st);
		$(".infoboxContainer").show();
		st = "";
		...
		$.each(allSuitesOnFloor, function (i2, eachSuite){
			...
			tabContent += '<div class="tab-content '+isActive+'" id="tab-'+i2+'">';
				tabContent += "<table class='table table-striped minPaddingtable'>";
					...
					tabContent += "<tr><td>Floor</td><td>"+eachSuite.floor_number+"</td><td>Available Area</td><td><span class='buildingNameOnInfobox'><b>"+numberWithCommaWithoutDecimal(eachSuite.suite_area, "", " "+cityAreaMeasurementUnit)+"</b></span></td></tr>";
					//tabContent += "<tr><td>Lease Type</td><td>"+eachSuite.lease_type+"</td><td>Space Type</td><td>"+eachSuite.space_type+"</td></tr>";
					
					tabContent += "<tr><td>Suite</td><td>"+eachSuite.suite_name+"</td><td>Currently Vacant</td><td></td></tr>";
		
					tabContent += "<tr><td>Class</td><td>"+eachSuite.class+"</td><td>Price per Sq Ft</td><td></td></tr>";
					
					//Created  //"+PrintOnlyDate(PrintIfNotNull(eachSuite.date_created))+"
					tabContent += "<tr><td>Age</td><td>"+eachSuite.year_difference+"</td><td>Annual Rent</td><td></td></tr>";
					
					//Status  //Active
					tabContent += "<tr><td></td><td></td><td>Lease Type</td><td style='background-color:"+classColor[eachSuite.lease_type]+"'>"+eachSuite.lease_type+"</td></tr>";
					...
					tabContent += "<tr><td colspan=2>Listing Broker</td><td></td><td aligh='right' style='float:right'>&nbsp;<span style='cursor:pointer;margin: 0px !important; padding-right: 0px !important;' id='copyURLButton' ><img height='32px;' width='32px;' src='images/link_24.png' /></span></td></tr>";
...
//In case if we want to revert
function prepareAvailableOfficeSpaceInfobox_OLD(idtbldg, index, details2, allSuitesOnFloor)
{
	var bldgName = "";
	var st = "";
	if(lastSelectedBuildingType == "Floorplan")
	{
		st = '<div style="margin: 5px; padding: 2px; margin-left: 0px !important; padding-left: 0px !important;">';
		st += '<a style="position: absolute;" href="javascript:void(0)" class="buildingNameOnInfobox" onclick="flyToBuildingCamera('+idtbldg+');">'+details2.sbuildingname+'</a>';
		st += "<span class='pull-right' style='position: absolute; right: 2% !important; margin-top: -5px !important;'><button class='btn btn-sm btn-secondary actionButtons' id='copyURLButton' ><img src='images/share.png' /></button></span>";
		st += '<br />';
		st += '</div>';
	}
	var details = [];
	...
	{
		st += "<table class='table table-striped minPaddingtable'>";
		st += "<tr><td>Suite</td><td><span class='buildingNameOnInfobox'>"+details[0]["suite_name"]+"</span></td>";
		if(details[0]["suite_area"] != "" && parseInt(details[0]["suite_area"]) > 0)
		{
			st += "<td>Area</td><td><span class='buildingNameOnInfobox'>"+numberWithCommaWithoutDecimal(details[0]["suite_area"], "", " "+cityAreaMeasurementUnit)+"</span></td>";
		}
		else
			st += "<td></td><td></td>";
		st += "</tr>";
		
		st += "<tr><td>Lease Type</td><td>"+details[0]["lease_type"]+"</td><td>Space Type</td><td>"+details[0]["space_type"]+"</td></tr>";
		st += "<tr><td>Company</td><td colspan='3'>"+details[0]["companyname"]+"</td></tr>";
		st += "</table>";
		st += prepareSuiteImagesTabStructure(details[0]["idtsuite"], idtbldg, parseInt(details2.floor_number), index, adminBaseUrl+details[0].image_path+details[0].image_name);
		if(details[0]["suite_description"] != "")
		{
	...
	else
	{
		...
		if(DefaultSuiteIndex != 'null' || DefaultSuiteIndex != null)
			isActive = "";
		var tabContent = '';
		$.each(allSuitesOnFloor, function (index, eachSuite){
			if(eachSuite.idtsuite == details[0].idtsuite)
			{
				isActive = " active ";
			}
			...
			tabContent += '<div class="tab-content '+isActive+'" id="tab-'+index+'">';
				tabContent += "<table class='table table-striped minPaddingtable'>";
					tabContent += "<tr><td>Suite</td><td><span class='buildingNameOnInfobox'>"+eachSuite.suite_name+"</span></td><td>Area</td><td><span class='buildingNameOnInfobox'>"+numberWithCommaWithoutDecimal(eachSuite.suite_area, "", " "+cityAreaMeasurementUnit)+"</span></td></tr>";
					tabContent += "<tr><td>Lease Type</td><td>"+eachSuite.lease_type+"</td><td>Space Type</td><td>"+eachSuite.space_type+"</td></tr>";
					tabContent += "<tr><td>Company</td><td>"+eachSuite.companyname+"</td><td></td><td></td></tr>";
				tabContent += "</table>";
				tabContent += prepareSuiteImagesTabStructure(eachSuite.idtsuite, idtbldg, floorNumber, index, adminBaseUrl+eachSuite.image_path+eachSuite.image_name);
...
function prepareSuiteImagesTabStructure(idtsuite, idtbldg, floorNumber, index, imagePath)
{
	...
	*/
	...
	var tabs = '<li class="nav-item"><a style="float:left; width: 85%;" class="nav-link active featureSheetLI" data-bs-toggle="tab" href="#floorplan-'+index+'">Feature Sheet</a><span style="float:right; width: 10%; cursor:pointer;" class="featureSheetImage toggle-icon ms-2" data-target="#floorplan-'+index+'" onclick="toggleFloorplan(this, event)">';
	...
	tabs += '</span></li>';
	var content = '<div class="tab-pane fade show active floorPlanImageDisplay" '+display+' id="floorplan-'+index+'">';
	...
	return "<ul class='nav nav-tabs' id='subTabs-"+index+"'>"+tabs+"</ul><div class='nav-tab-content mt-2'>"+content+"</div>";
}

function toggleFloorplan(el, event) {
    event.stopPropagation(); // prevent tab switching
    if($(".floorPlanImageDisplay").css("display") == "block")
	{
		$(".floorPlanImageDisplay").css("display", "none");
		$(".featureSheetToggleIcon").attr("src", "images/Expand.png");
		window.featureFloorPlanCollapsed = false;
	}
	else
	{
		$(".floorPlanImageDisplay").css("display", "block");
...
function getMarketSalesDataCalgary()
{
	if(window.calgaryOfficeSale == null || window.calgaryOfficeSale.length == 0)
	{
		$.ajax({
			method: "POST",
			url: "controllers/buildingController.php",
			data: { param : "getMarketSalesDataCalgary", idtcity: 2}
		}).done(function (data) {
			data = $.parseJSON( data.trim() );
			console.log(data);
			...
			highlightCalgaryOfficeMarketSales();
...
window.availableOfficeSpaceSummary = null;
window.availableOfficeSpacePrimitives = [];
function getAvailableOfficeSpace()
{
...
{
	//console.log("in splitPolygonIntoPieces()");
					...
					if(!isNaN(parseFloat(eachSuite.FloorNumber)) && typeof eachSuite.FloorNumber != "undefined" && eachSuite.FloorNumber != null)
					{
						var ent = viewer.scene.groundPrimitives.add(new Cesium.ClassificationPrimitive({
								geometryInstances : new Cesium.GeometryInstance({
									...
									id : "arealyticSuite-"+eachSuite.idtbuilding+"-"+indexes+"-"+cntr,
								}),
								classificationType : Cesium.ClassificationType.CESIUM_3D_TILE
							}));
...
function highlightAvailableOfficeSpace()
{
	...
	window.lastHolesString = '';
		$.each(availableOfficeSpace, function (index, eachSuite){
			...
			//console.log("------------------");console.log(eachSuite.coords);
...
function prepareLogoAndSqftLabels(id, lat, lon, ht, imagePath, sqftValue) {
	...
	*/
	...
	viewer.entities.add({
		...
		label: {
			text: numberWithCommaWithoutDecimal(sqftValue, "", " " + cityAreaMeasurementUnit),
			font: "30px Helvetica",
			horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
			disableDepthTestDistance: Number.POSITIVE_INFINITY,
			fillColor: Cesium.Color.BLACK,
...
function highlightCalgaryOfficeMarketSales()
{
	//viewer.entities.removeById("FogEffectEntity");
	//viewer.entities.removeById("FogEffectEntityPreload");
	...
	$.each(calgaryOfficeSale, function (cntr, eachSuite){
		...
		if(eachSuite.sold_price != null && eachSuite.sold_price != "")
		{
			var pts = eval("["+eachSuite.coords+"]");
			var pt1 = getCentroid(pts);
			var position = Cesium.Cartesian3.fromDegrees(pt1[0], pt1[1], parseInt(window.TempBldgData[eachSuite.idtbuilding].altitude) + parseInt(window.TempBldgData[eachSuite.idtbuilding].floors * 4));
	...
	viewer.entities.removeById("FogEffectEntity");
	viewer.entities.removeById("FogEffectEntityPreload");
	eval("viewer.entities.add({ id: 'FogEffectEntity', polygon: { hierarchy: { positions: Cesium.Cartesian3.fromDegreesArray("+cityBoundaries[2]+"), holes: eval(["+window.lastHolesString+"]) }, material: Cesium.Color."+getDarkOverlayColor()+".withAlpha(0.5), classificationType: Cesium.ClassificationType.CESIUM_3D_TILE, }, }) ");
	executeDefaultEffectsAndCamera();
}
						...
						if(!isNaN(parseFloat(eachSuite.FloorNumber)) && typeof eachSuite.FloorNumber != "undefined" && eachSuite.FloorNumber != null)
						{
							var ent = viewer.scene.groundPrimitives.add(new Cesium.ClassificationPrimitive({
									geometryInstances : new Cesium.GeometryInstance({
										...
										id : "arealyticSuitePriceSQM-"+eachSuite.idtbuilding+"-"+indexes+"-"+cntr,
									}),
									classificationType : Cesium.ClassificationType.CESIUM_3D_TILE
								}));
			...
			fetch(apiUrl)
				...
				.then(data => {
					if (data.shortURL) {
						...
						const fullUrl = "https://www.visgrid.com/measure/app10/?q=" + shortCode;
...
function openFullScreenImage(idtbuilding, floorNumber, index, imgCounter = 0, imgType = "", idtsuite = 0, floorPlanView = "No")
{
	...
	{
		st += "<div style='float:left; position: absolute;'><b>Area:</b> <span class='buildingNameOnInfobox'>"+numberWithCommaWithoutDecimal(details[index]["suite_area"], "", " "+cityAreaMeasurementUnit)+"</span></div><br clear='all'>";
	}
	//st += "<div style='float:left;'><b>Note:</b> <span class='buildingNameOnInfobox'>"+details[index]["suite_description"]+"</span></div>";
...
function openFullScreenImageAOS(idtbuilding, floorNumber, index, imgCounter = 0, imgType = "", idtsuite = 0)
{
	...
	if(details["suite_area"] != "" && parseInt(details["suite_area"]) > 0)
	{
		st += "<div style='float:left; position: absolute;'><b>Area:</b> <span class='buildingNameOnInfobox2'>"+numberWithCommaWithoutDecimal(details["suite_area"], "", " "+cityAreaMeasurementUnit)+"</span></div><br clear='all'>";
	}
